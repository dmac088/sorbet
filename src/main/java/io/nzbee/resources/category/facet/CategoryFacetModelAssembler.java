...
public class CategoryFacetModelAssembler extends RepresentationModelAssemblerSupport<EntityFacetHierarchical, CategoryFacetModel> {
	...
	public CollectionModel<CategoryFacetModel> toCollectionModel(List<EntityFacetHierarchical> collection) {
		return CollectionModel.of(collection.stream().map(c -> this.toModel(c)).collect(Collectors.toList()));
	}
